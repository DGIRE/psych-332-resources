/**
 * PSYCH 332 — Lightweight Privacy-Preserving Analytics
 * =====================================================
 ...
 *                    click_external_tool, click_brain_explorer,
 *                    click_notebooklm, click_zoom, click_canvas
 ...
 *   "page_title": "PSYCH 332 Resources",
...
(function () {
  ...
  function getSessionId() {
    try {
      var id = sessionStorage.getItem('_p332_sid');
      if (!id) {
        id = 's_' + Math.random().toString(36).substring(2, 12);
        sessionStorage.setItem('_p332_sid', id);
      }
      return id;
    } catch (e) {
  ...
  function classifyClick(href, el) {
    ...
    // Brain Explorer
    if (lower.indexOf('brain-explorer') !== -1 || lower.indexOf('Brain-Explorer') !== -1) return 'click_brain_explorer';
    if (el && el.getAttribute('data-slot-type')) return 'click_brain_explorer';

    // NotebookLM
    if (lower.indexOf('notebooklm.google.com') !== -1) return 'click_notebooklm';

    // Zoom
    if (lower.indexOf('zoom.us') !== -1) return 'click_zoom';

    // Canvas LMS
    if (lower.indexOf('canvas.uw.edu') !== -1) return 'click_canvas';

    // Other external (Google Drive, Colab, PubMed, Allen Brain, etc.)
